<template>
  <view class="page_tree_hole_release diy_table diy_table_tml" id="tree_hole_release_table">
    <!-- 筛选模块(开始) -->
    <view>
          ...
          <view>
            <view class="">
              ...
              <uni-forms :modelValue="query">
                    <uni-forms-item label="内容标题" name="content_title">
                      <uni-easyinput type="text" v-model="query.content_title" placeholder="内容标题" />
                    </uni-forms-item>
                    <uni-forms-item label="内容分类" name="content_classification">
                  <uni-data-select placeholder="请选择"
            v-model="query['content_classification']"
            :localdata="list_content_classification"
        ></uni-data-select>
                         </uni-forms-item>
                      </uni-forms>
              <!-- /搜索栏 -->
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/tree_hole_release/view?" v-if="user_group == '管理员' || $check_action('/tree_hole_release/table','add') || $check_action('/tree_hole_release/view','add')">添加</navigator>
						...
						<view class="view">
							<view class="diy_title" v-if="true">
								...
								<span>
									...
									<view class="col" v-if="1 && $check_field('get','content_title')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>内容标题</span>
							</view>
								...
								<span>
									...
									<view class="col" v-if="1 && $check_field('get','content_classification')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>内容分类</span>
							</view>
															<view class="diy_field diy_text text">
								<span>
									{{ o["content_classification"] }}
								</span>
								</view>
													</view>
					</view>
									<view class="col" v-if="1 && $check_field('get','image_content')">
						<view class="view">
							<view class="diy_title" v-if=" false">
								<span>图片内容</span>
							</view>
															<view class="diy_field diy_img">
									<img :src="$fullUrl(o['image_content'])" width="100%" height="100" />
								</view>
													</view>
					</view>
									<view class="col" v-if="0 && $check_field('get','video_content')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>视频内容</span>
							</view>
								...
								<span>
									...
									<view class="col" v-if="0 && $check_field('get','text_content')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>文字内容</span>
							</view>
				...
				<view class="col">
					<view class="view">
						<view class="create_time_block diy_filed diy_time">
                          ...
                          <button class="delete_a_btn" @click="delInfo(i)" v-if="user_group == '管理员' || $check_action('/tree_hole_release/table','del') || $check_action('/tree_hole_release/view','del')">删除</button>
                        </view>
					</view>
				</view>
				<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/tree_hole_release/view?' + field + '=' + o[field]"
						v-if="user_group == '管理员' || $check_action('/tree_hole_release/table','set') || $check_action('/tree_hole_release/view','set') || $check_action('/tree_hole_release/view','get')" class="btn btn-primary">详情</navigator>
					                                        																		<!--跨表按钮-->
														<!--	<a type="button" size="mini" class="btn btn-primary" v-if="$check_action('/content_reporting/view','add')" @click="to_form('/pages/content_reporting/view',o)">
									<span>举报</span>
								</a>-->
...
<script>
	...
	export default {
		...
		data() {
			return {
				// 获取数据地址
				url_get_list: "~/api/tree_hole_release/get_list?like=0",
				url_del: "~/api/tree_hole_release/del?",
                ...
                list_user_publish_users: [],
                                // 内容分类列表
                list_content_classification: [{value:"",text:"全部"}],
                                			}
		},
		methods: {
          search_() {
