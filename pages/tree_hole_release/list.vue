<template>
	<view class="page_tree_hole_release diy_list diy_list_page div_list_tml" id="tree_hole_release_list">
		<!-- 筛选模块(开始) -->
		<view>
			<view class="container">
				<view>
					<view>
						<view class="">
							<!-- 搜索栏 -->
														<uni-search-bar placeholder="搜索内容标题" @confirm="search_" @cancel="search_cancel('content_title')" cancelText="取消"
											@input="input($event, 'content_title')">
				...
				<view>
					<view>
						<view class="tab_view">
							<!-- 筛选栏 -->
																		<uni-section title="内容分类" type="line">
							<uni-data-select
									@change="change_content_classification"
			...
			<view class="container">
				<view v-for="(o,i) in list" class="row box_style">
					...
					<view class="uni-padding-wrap uni-common-mt">
						<navigator :url="'./details?tree_hole_release_id=' + o['tree_hole_release_id']" class="btn btn-primary">查看详情</navigator>
...
<script>
	...
	get_praise(list){
		if (list) {
			...
			this.$get("~/api/praise/list_group?groupby=source_id&source_table=tree_hole_release", {}, (res) => {
				if (res.result && res.result.list) {
					res.result.list.map((o) => {
						for (var i = 0; i < list.length; i++) {
							var oj = list[i];
							if (oj["tree_hole_release_id"] === o["source_id"]) {
	...
	get_user_publish_users(id){
		...
		return ret;
						...
						// 改变分类标签
	change_content_classification(val) {
		this.query.content_classification = val;
		this.search_();
	},
