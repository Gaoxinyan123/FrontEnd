<template>
	<view class="container account_login">
		...
		<view class="wrapper">
      		<view class="top-sign-title">树洞微信小程序</view>
...
<script>
	...
	import wx_open from "@/libs/store/wx_open.json";
						...
						uni.login({
							...
							success: (res) => {
								...
								if (res.errMsg == 'login:ok') {
									...
									that.$get('https://api.weixin.qq.com/sns/jscode2session', {
										appid: wx_open.appid,
										...
										if (res.openid){
											//登录
											that.$post('~/api/user/open_id_login?', {open_id:res.openid,username:that.form.username,password:that.form.password}, (res) => {
												if (res.result && res.result.obj) {
													let obj = res.result.obj;
													// 缓存token
													uni.db.set("token", obj.token);
													// 存储用户信息
													that.$store.commit("user_set", obj);
													// 获取权限集
													that.$get_auth(this.user.user_group);
													// 前往首页
													that.$nav("/pages/index/index");
												} else if (res.error) {
													this.$toast(res.error.message, "error");
												}
											});
