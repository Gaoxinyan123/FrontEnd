<template>
  <view class="page_content_reporting diy_table diy_table_tml" id="content_reporting_table">
    <!-- 筛选模块(开始) -->
    <view>
          ...
          <view>
            <view class="">
              ...
              <uni-forms :modelValue="query">
                    ...
                    <uni-forms-item label="举报类型" name="report_type">
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/content_reporting/view?" v-if="user_group == '管理员' || $check_action('/content_reporting/table','add') || $check_action('/content_reporting/view','add')">添加</navigator>
                            ...
                            <span>发布用户</span>
								...
								<text>
									{{ get_user_publish_users(o['publish_users']) }}
													...
													<view v-if="0 && $check_field('get','report_users')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>举报用户</span>
                          </view>
											<view class="diy_field diy_uid text">
								<text>
									{{ get_user_report_users(o['report_users']) }}
								</text>
							</view>
										</view>
					</view>
													<view v-if="1 && $check_field('get','content_title')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>内容标题</span>
                          </view>
								...
								<text>
									{{ o["content_title"] }}
													...
													<view v-if="1 && $check_field('get','report_type')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>举报类型</span>
                          </view>
								...
								<text>
									{{ o["report_type"] }}
													...
													<view v-if="0 && $check_field('get','reporting_details')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>举报详情</span>
                          </view>
                  ...
                  <view class="col">
                    <view class="view">
                      <view class="create_time_block diy_filed diy_time">
                        ...
                        <button class="delete_a_btn" @click="delInfo(i)" v-if="user_group == '管理员' || $check_action('/content_reporting/table','del') || $check_action('/content_reporting/view','del')">删除</button>
                      </view>
                    </view>
                  </view>
					<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/content_reporting/view?' + field + '=' + o[field]"
                               v-if="user_group == '管理员' || $check_action('/content_reporting/table','set') || $check_action('/content_reporting/view','set') || $check_action('/content_reporting/view','get')" class="btn btn-primary">详情</navigator>
	            					</view>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		data() {
			return {
				// 获取数据地址
				url_get_list: "~/api/content_reporting/get_list?like=0",
				url_del: "~/api/content_reporting/del?",
                ...
                list_user_report_users: [],
                                // 举报类型列表
                list_report_type: [{value:"",text:"全部"}],
          ...
          reset: function reset() {
            ...
            if(user_group != "管理员"){
                  ...
                  sqlwhere+= "publish_users = " + this.user.user_id + " or ";
                                        ...
                                        if(user_group=="注册用户"){
                  sqlwhere+= "report_users = " + this.user.user_id + " or ";
              }
