<template>
  <view id="home" class="home--new">
    ...
    <Card
      class="list_diy"
      title="树洞发布推荐"
      url="/pages/tree_hole_release/list"
      :list="list_tree_hole_release"
      v-if="$check_action('/tree_hole_release/list', 'get')"
    >
	  <list_tree_hole_release :list="list_tree_hole_release"></list_tree_hole_release>
...
<script>
				...
				import list_tree_hole_release from "@/components/diy/list_tree_hole_release.vue";
  ...
  methods: {
    ...
    toToggle() {
      this.isAdmin = !this.isAdmin;
		...
		*  获取树洞发布
		*/
		get_tree_hole_release() {
			let url = getApp().globalData.host + "/api/tree_hole_release/get_list?orderby=hits desc";
			let param = { "page": 1, "size": 12 };
			this.$get(url, param, (json) => {
				if (!json || !json.result || !json.result.list) {
					uni.showToast({ title: '树洞发布获取失败', icon: 'none' });
					return;
				}
				this.list_tree_hole_release = json.result.list;
				this.list_tree_hole_release.map((o)=>{ o["praise_len"]; });
				this.get_praise(this.list_tree_hole_release ,"tree_hole_release" ,"tree_hole_release_id");
			});
		},
    ...
    get_slides() {
      this.$get(getApp().globalData.host + "/api/slides/get_list?", {}, (json) => {
        if (!json || !json.result || !json.result.list) {
          uni.showToast({ title: '轮播图获取失败', icon: 'none' });
          return;
        }
        this.list_slide = json.result.list;
        this.list_slide.map((o) => { o["praise_len"]; });
        this.get_praise(this.list_slide, "slides", "slides_id");
      });
    },
    ...
    get_menu() {
      ...
      this.$get(
        ...
        (json) => {
          if (!json || !json.result || !json.result.list) {
            uni.showToast({ title: '菜单获取失败', icon: 'none' });
            return;
          }
          this.list_menu = json.result.list;
          this.list_menu.map((o) => { o["praise_len"]; });
          this.get_praise(this.list_menu, "auth", "auth_id");
        }
      );
    },
     ...
     */
    get_article() {
      this.$get(getApp().globalData.host + "/api/article/get_list?", { page: 1, size: 6 }, (json) => {
        if (!json || !json.result || !json.result.list) {
          uni.showToast({ title: '文章获取失败', icon: 'none' });
          return;
        }
        this.list_article = json.result.list;
        this.list_article.map((o) => { o["praise_len"]; });
    ...
    get_notice() {
      this.$get(getApp().globalData.host + "/api/notice/get_list?", { page: 1, size: 3 }, (json) => {
        if (!json || !json.result || !json.result.list) {
          uni.showToast({ title: '公告获取失败', icon: 'none' });
          return;
        }
        this.list_notice = json.result.list;
        this.list_notice.map((o) => { o["praise_len"]; });
    ...
    get_praise(list, table, idName) {
      if (list) {
        ...
        this.$get(
            getApp().globalData.host + "/api/praise/list_group?source_table=" + table + "&groupby=source_id",
            {},
            (res) => {
